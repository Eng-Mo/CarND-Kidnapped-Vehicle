...
int main()
{
  ...
  h.onMessage([&pf,&map,&delta_t,&sensor_range,&sigma_pos,&sigma_landmark,&update_out](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
...
//        	cout<<"intit status"<<pf.initialized()<<endl;
          ...
          if (!pf.initialized()) {
			...
			bool t =pf.initialized();
//			update_out<<"status initialised "<<t<<endl;
...
//			 update_out<<"status predection "<<t<<endl;
...
//        	cout<<"total landmark="<<map.landmark_list.size()<<endl;
